...
describe('Fleets', () => {
  describe('Given a Get By ID for get Fleet', () => {
    describe('WHEN there is a Fleet with the specified valid idRental and idReserve', () => {
      let response;

      beforeEach(async () => {
        response = await request(app).get(`/api/v1/rental/${idRental}/fleet/${idFleet}`);
      });
    ...
    describe('WHEN trying to get a Fleet with a Rental ID NotFound', () => {
      ...
      beforeEach(async () => {
        response = await request(app).get(`/api/v1/rental/${idRentalNotFound}/fleet/${idFleet}`);
      });
    ...
    describe('WHEN trying to get a Fleet with a Rental ID Invalid', () => {
      ...
      beforeEach(async () => {
        response = await request(app).get(`/api/v1/rental/${idRentalInvalid}/fleet/${idFleet}`);
      });
    ...
    describe('WHEN trying to get a Fleet with a Fleet ID NotFound', () => {
      let response;
      const idFleetNotFound = '61a1ae0cfabfbb9e30e03f78';
      beforeEach(async () => {
        response = await request(app).get(`/api/v1/rental/${idRental}/fleet/${idFleetNotFound}`);
      });
    ...
    describe('WHEN trying to get a Fleet with a Fleet ID Invalid', () => {
      let response;
      const idFleetInvalid = '61a1ae0cfabfbb9e30e03f783';
      beforeEach(async () => {
        response = await request(app).get(`/api/v1/rental/${idRental}/fleet/${idFleetInvalid}`);
      });
