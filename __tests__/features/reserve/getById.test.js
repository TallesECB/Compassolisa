...
describe('Reserves', () => {
  describe('Given a Get By ID for get Reservation', () => {
    describe('WHEN there is a Reserve with the specified valid idRental and idReserve', () => {
      let response;

      beforeEach(async () => {
        response = await request(app)
          .get(`/api/v1/rental/${idRental}/reserve/${idReserve}`)
          .set('Authorization', `Bearer ${token}`);
      });
    ...
    describe('WHEN trying to get a Reserve with a Rental ID NotFound', () => {
      ...
      beforeEach(async () => {
        response = await request(app)
          .get(`/api/v1/rental/${idRentalNotFound}/reserve/${idReserve}`)
          .set('Authorization', `Bearer ${token}`);
      });
    ...
    describe('WHEN trying to get a Reserve with a Rental ID Invalid', () => {
      ...
      beforeEach(async () => {
        response = await request(app)
          .get(`/api/v1/rental/${idRentalInvalid}/reserve/${idReserve}`)
          .set('Authorization', `Bearer ${token}`);
      });
    ...
    describe('WHEN trying to get a Reserve with a Reserve ID NotFound', () => {
      let response;
      const idReserveNotFound = '61a1ae0cfabfbb9e30e03f78';
      beforeEach(async () => {
        response = await request(app)
          .get(`/api/v1/rental/${idRental}/reserve/${idReserveNotFound}`)
          .set('Authorization', `Bearer ${token}`);
      });
    ...
    describe('WHEN trying to get a Reserve with a Reserve ID Invalid', () => {
      let response;
      const idReserveInvalid = '61a1ae0cfabfbb9e30e03f783';
      beforeEach(async () => {
        response = await request(app)
          .get(`/api/v1/rental/${idRental}/reserve/${idReserveInvalid}`)
          .set('Authorization', `Bearer ${token}`);
      });
